 ...
 */
namespace Custform\Newmodule\Ui\Component\Listing\Column;
...
{
    /** Url path */
    const URL_PATH_EDIT = 'newmodule/newmodule/edit';
    const URL_PATH_DELETE = 'newmodule/newmodule/delete';
        ...
        if (isset($dataSource['data']['items'])) {
            foreach ($dataSource['data']['items'] as & $item) {
                ...
                if (isset($item['entity_id'])) {
                    ...
                    // echo $this->urlBuilder->getUrl($this->editUrl, ['entity_id' => $item['entity_id']]);die;



                    $entity_id = $this->getEscaper()->escapeHtml($item['entity_id']);
                    $item[$name]['delete'] = [
                        'href' => $this->urlBuilder->getUrl(self::URL_PATH_DELETE, ['entity_id' => $item['entity_id']]),
                        'label' => __('Delete'),
                        'confirm' => [
                            'entity_id' => __('Delete %1', $entity_id),
                            'message' => __('Are you sure you want to delete a %1 record?', $entity_id),
                        ],
                        'post' => true,
                    ];
                }
